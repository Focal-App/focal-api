defmodule FocalApiWeb.ClientControllerTest do
  ...
  describe "show" do
    ...
    test "renders error when user is logged in but not authorized to view", %{
      conn: conn,
      client: client,
      user: user
    } do
  ...
  describe "show_all_client_data" do
    ...
    test "shows all related data for client", %{conn: conn, client: client} do
      ...
      _task2 =
        TestHelpers.task_fixture(%{client_id: client.id, is_completed: false, event_id: nil})
  ...
  describe "update client" do
    ...
    test "renders client when data is valid", %{
      conn: conn,
      client: %Client{uuid: uuid, id: id} = _client
    } do
      _contact = TestHelpers.contact_fixture(%{client_id: id})
      client = TestHelpers.preloaded_client(uuid)
    ...
    test "renders error when data is invalid", %{
      conn: conn,
      client: %Client{uuid: uuid} = _client
    } do
      client = TestHelpers.preloaded_client(uuid)
    ...
    test "renders error when user is logged in but not authorized to make the change", %{
      conn: conn,
      client: client,
      user: user
    } do
  ...
  describe "index_by_user" do
    ...
    test "lists all clients for a user", %{conn: conn, client: client} do
      _contact = TestHelpers.contact_fixture(%{client_id: client.id})
      client = TestHelpers.preloaded_client(client.uuid)
